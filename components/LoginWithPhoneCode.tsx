...
export default function LoginWithPhoneCode({ navigationRef }) {
  ...
  const renderCell = ({ index, symbol, isFocused }) => {
    ...
    return (
      <AnimatedText
        key={index}
        style={[CodeStyles.cell, animatedCellStyle]}
  ...
  return (
    <View style={{ display: 'flex' }}>
      ...
      <Animated.View style={[styles.UsernameAPassword, { transform: [{ translateY: phoneNumLocate.y }] }]}>
        <View style={{ backgroundColor: '#050505', display: !CodeVisible ? 'flex' : 'none', flexDirection: 'row', height: 50, width: '100%', position: 'absolute', top: 0 }}>
          ...
          <Input inputStyle={styles.UAPInputStyle} style={styles.UAPInput} placeholder='Phone Number' value={UserName} onChange={v => SetUserName(v)}></Input>
        </View>
        ...
        <Text style={{ fontSize: 15, color: '#ff5819', marginTop: 50, height: 20, alignSelf: 'center', display: NumWarningVisible ? "flex" : "none" }}>a number of length 11 is required</Text>
